...
tldr: "开放式桌面操作要求智能体理解自然语言并适应动态环境和执行失败。ACE框架利用零样本工作流推理，通过视觉接地接口和可重用的拾放原语连接语义与物理控制，采用掩码介导机制指定目标并闭环验证。在逻辑复杂的长时任务中，ACE在方程形成和约束检索任务上分别达到50%和70%成功率，展示了显式推理与掩码控制的鲁棒性。"
source: arxiv
...
motivation: 现有端到端方法难以完成逻辑复杂的桌面操作任务，需要新框架实现零样本推理和自适应控制。
method: 提出ACE，结合工作流推理与视觉掩码介导接口，通过多时间尺度内存实现闭环验证和在线自适应。
result: "ACE在零样本下于方程形成任务达50%成功率，约束检索任务达70%成功率，远超端到端基线。"
conclusion: 显式工作流推理和掩码介导控制为可适应机器人操作提供鲁棒实用路径。
---

## 摘要
开放式桌面操作要求智能体不仅理解自然语言，还要适应动态环境和执行失败。我们提出ACE（具身操作的智能体控制），一个用于从自然语言进行桌面拾放的零样本工作流推理框架。无需依赖直接的底层动作映射，ACE将智能体工作流推理与两种面向机器人的可执行技能相结合：视觉基础接口和可复用的拾放原语。为了桥接语义推理与物理控制，活跃的子目标被嵌入到掩码介导的视觉-动作接口中。这个统一的掩码指定目标对象和目的地，随时间被跟踪，暴露以供人类验证，并最终传递给与任务无关的下游策略执行。关键的是，ACE在由多时间尺度记忆支持的闭环中运行。动作执行后，系统自动验证预期子目标是否成功，并使用结果来推进、重试、修复或重新规划。这使得能够在线适应用户纠正、场景变化和物理故障。我们在逻辑复杂、长时域任务上评估ACE，包括使用数字立方体进行零样本多步方程形成和基于约束的对象检索。ACE在新颖的语义约束和随机桌面场景上展示了任务级零样本泛化，无需针对特定任务的再训练。具体而言，当标准的端到端基线难以完成这些逻辑要求高的任务时，ACE在方程形成中达到50%的成功率，在约束检索中达到70%的成功率。这一对比表明，明确的工作流推理和掩码介导控制为适应性的机器人操作提供了一条稳健、实用的路径。
